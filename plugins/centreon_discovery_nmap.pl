...
$fatpacked{"discovery/nmap/mode/fastscan.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'DISCOVERY_NMAP_MODE_FASTSCAN';
  #
  ...
  package discovery::nmap::mode::fastscan;
  ...
  sub new {
      ...
      $options{options}->add_options(
          arguments => {
              ...
              "command-options:s" => { name => 'command_options', default => '-sP -T4 ' },
          }
      );
  ...
  sub check_options {
      ...
      if (defined($self->{option_results}->{range}) && $self->{option_results}->{range} ne '') {
          if ($self->{option_results}->{range} =~ /\d+\.\d+\.\d+\.\d+\/?.\d+?/) {
              $self->{option_results}->{command_options} .= $self->{option_results}->{range};
          } else {
              $self->{output}->add_option_msg( short_msg =>
                  "Wrong range IP address '".$self->{option_results}->{range}."'." );
              $self->{output}->option_exit();
          }
      }
  }
  ...
  sub run {
      ...
      my $command = '';
      if (defined($self->{option_results}->{command_path})) {
          $command .= $self->{option_results}->{command_path};
      }
      if (defined($self->{option_results}->{command})) {
          $command .= $self->{option_results}->{command};
      }
      if (defined($self->{option_results}->{command_options})) {
          $command .= " ".$self->{option_results}->{command_options};
      }
      ...
      open (LINE, $command."|") || die print "{\"error\": \"Error to execute command: $command $! \"}";
  
      foreach (<LINE>) {
          if ($_ =~ /Nmap scan report for (.*)/) {
              $mac_address = '';
              $type = '';
              if ($1 =~ /(.*) \((.*)\)/) {
                  $fqdn = $1;
              ...
              my %scanArray = (ip_address => $ip_address);
              if ($mac_address ne '') {
                  $scanArray{'mac_address'} = $mac_address;
              }
              if ($type ne '') {
                  $scanArray{'type'} = $type;
              }
  ...
  Check available Up elements on range IP address
  
  =over 8
  
  =item B<--range>
  
  IP range Address, for example 192.168.0.0/24
  ...
  Command to get information (Default: 'nmap').
...
$fatpacked{"discovery/nmap/plugin.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'DISCOVERY_NMAP_PLUGIN';
  #
  # Copyright 2016 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  sub new {
      ...
      $self->{version} = '0.1';
      %{$self->{modes}} = (
          'fastscan' => 'discovery::nmap::mode::fastscan',
      );
  ...
  Discovery of network elements to add those to the monitoring
